      ...
      <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
        ...
        <div className="flex items-center text-gray-600 mb-4">
          ...
          <span className="ml-2 font-bold text-sm">
            {data.confidence !== undefined ? data.confidence.toFixed(1) : "N/A"}%
          </span>
        </div>
  ...
  const [chatHistory, setChatHistory] = useState<ChatMessage[]>([
    {
      ...
      data: {
        answer:
          "Hello! I'm your AI Financial Advisor. I can forecast stock movements, analyze market performance, and answer questions based on the latest news. What would you like to know?",
        sources: [],
      },
    },
  ])
  ...
  const handleSubmit = async (e: HandleSubmitEvent): Promise<void> => {
    ...
    try {
      ...
      const errorResponse: ChatMessage = {
        type: "ai",
        data: { answer: "Sorry, I'm having trouble connecting to the server. Please try again later.", sources: [] },
      }
      ...
      <main className="flex-1 ml-64 flex flex-col h-screen">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {chatHistory.map((message, index) => (
            ...
            >
              ...
              <div
                className={`max-w-xl p-4 rounded-2xl ${message.type === "user" ? "bg-blue-600 text-white rounded-br-none" : "bg-white border border-gray-200 rounded-bl-none"}`}
              >
                {message.type === "user" ? (
                  <p>{message.text}</p>
                ) : (
                        ...
                        <div className="flex flex-wrap gap-2">
                          {message.data.sources.map((source, i) => (
                            <a
                              ...
                              key={i}
                              className="text-xs bg-gray-100 text-blue-700 px-2 py-1 rounded-md hover:bg-blue-100 transition-colors"
                            >
        ...
        <div className="p-6 bg-white border-t border-gray-200">
          <div className="grid gap-2 mb-3">
            <div className="flex flex-wrap gap-2">
              {quickQuestions.map((q) => (
                <button
                  key={q}
                  onClick={() => handleSubmit({ preventDefault: () => {}, target: { value: q } })}
                  className="px-3 py-1.5 bg-gray-100 border border-gray-200 rounded-full text-xs font-medium text-gray-700 hover:bg-blue-50 hover:border-blue-300 transition-all"
                >
                  {q}
                </button>
              ))}
          ...
          <form
            onSubmit={(e) => {
              e.preventDefault()
              handleSubmit({ preventDefault: () => {}, target: { value: query } })
            }}
            className="relative"
          >
            <input
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 bg-blue-600 text-white rounded-md p-2 hover:bg-blue-700 transition-all disabled:bg-blue-300 disabled:cursor-not-allowed"
            >
              <Send size={18} />
            </button>
          </form>
